...
def transform_profile_data(raw_profile, symbol):
    ...
    if 'MarketCapitalization' in raw_profile and isinstance(raw_profile['MarketCapitalization'], (int, float)):
        return raw_profile

    try:
        def parse_num(val):
            if val is None or val == 'None' or val == 'N/A': return 0
            try: return float(val)
            except: return 0

        clean_data = {
            'Symbol': symbol,
            'Name': raw_profile.get('Name', raw_profile.get('longName', symbol)),
            'Industry': raw_profile.get('Industry', raw_profile.get('industry', 'N/A')),
            'MarketCapitalization': parse_num(raw_profile.get('MarketCapitalization', raw_profile.get('marketCap', 0))),
            'PERatio': parse_num(raw_profile.get('PERatio', raw_profile.get('trailingPE', 0))),
            '52WeekHigh': parse_num(raw_profile.get('52WeekHigh', raw_profile.get('fiftyTwoWeekHigh', 0))),
            '52WeekLow': parse_num(raw_profile.get('52WeekLow', raw_profile.get('fiftyTwoWeekLow', 0))),
            'Description': raw_profile.get('Description', raw_profile.get('longBusinessSummary', 'No description available.'))
        }
        return clean_data
    except Exception as e:
        print(f"Error transforming profile: {e}")
        return {}
...
def transform_all(raw_daily_data, raw_profile_data, symbol="Unknown"):
    ...
    if raw_daily_data is None or raw_daily_data.empty:
        return None, clean_profile, None, None
    ...
    try:
        ...
        if not last_year_data.empty:
            clean_profile['52WeekHigh'] = last_year_data['high'].max()
            clean_profile['52WeekLow'] = last_year_data['low'].min()
    except Exception:
        pass
