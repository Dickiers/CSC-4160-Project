...
def parse_firestore_field(field):
    """Recursively parses Firestore JSON field format to Python types."""
    key = list(field.keys())[0]
    value = field[key]
    if key == "mapValue":
        return {k: parse_firestore_field(v) for k, v in value["fields"].items()}
    elif key == "arrayValue":
        return [parse_firestore_field(v) for v in value.get("values", [])]
...
def get_current_data(symbol, headers):
    """Fetches existing history from Firestore to merge with new data."""
    try:
        ...
        if response.status_code == 200:
            ...
            if "fields" in doc:
                ...
                if daily_raw:
                    ...
                    return df, parse_firestore_field(fields.get("clean_profile", {}))
        return None, None
    except Exception as e:
        print(f"Warning: Could not fetch existing data for {symbol}: {e}")
        return None, None
...
def save_to_firebase_rest(symbol, daily_data, profile, performance, headers):
    try:
        ...
        logger.error(f"Save Error {symbol}: {e}")
        return False

def lambda_handler(event, context):
    print("⚡ Lambda Triggered: Starting Daily Update...")
    ...
    for symbol in COMPANIES:
        try:
            ...
            if new_df is not None and not new_df.empty:
                ...
                trans_daily, trans_profile, trans_perf, _ = transform.transform_all(combined_df, existing_profile, symbol)
